...
try {
	if (session != null) {
int ind=0; Connection con = DB.getCon(); String
uid=(String)session.getAttribute("id"); String
name=request.getParameter("name"); String
pwd=request.getParameter("password"); String
tpwd=request.getParameter("tpassword"); String
cid=request.getParameter("cid"); String
bname=request.getParameter("bname"); String
atype=request.getParameter("atype"); float bal=1000;int status=0; long
acc=0l; System.out.println("bname: "+bname+" "+uid); PreparedStatement
st; ResultSet r; st = con.prepareStatement("select * from customer where cid=? and bname=?"); st.setString(1,cid); st.setString(2, bname); r =
st.executeQuery(); if(r.next()) {%>
<center>
